...
Output: a structured basic-info report at `./deals/processing/han-wu-ji/YYYYMMDD_han-wu-ji_basic_info.md`, with a Phase 1 scorecard appended.

The output directory is auto-created with `mkdir -p` under your current working directory (here `./deals/`); no `./workspace/` setup is required. If the CWD is not writable, the command falls back to read-only mode and prints the report to chat.
...
Requires a prior `deal-analysis` run on the same company (it reads `./<slug>/evidence.md`). Hard-fails if evidence is missing.
...
Output: per-sector intel briefing markdown at `./intel/YYYYMMDD-news-scan.md`.
...
**Default input directory**: `./portfolio/<slug>/`. Step 3 auto-scans this directory for filename patterns matching each material type (`*合并报表*.pdf` / `*财务报表*.pdf` / `*投后跟进报告*.{md,pdf,docx}` / `*董事会*.md` / `*访谈*.md` / `*新闻*.md` etc.) and presents them via `AskUserQuestion` with two options:
...
- `./portfolio/<slug>/project_baseline.yml` — one-time investment terms (基金主体 / 投资协议日期 / 投前估值 / 投资金额 / 持股比例 / 投资完成时点股权结构表). First run prompts; subsequent runs reuse without asking.
- `./portfolio/<slug>/competitors.yml` — editable competitor list. Step 2 lets you add/remove/reorder before the parallel dispatch.
...
| Building a watchlist file outside the `./portfolio/` tree | `/analyst-deal:competitor-enricher`  |
...
/analyst-deal:competitor-enricher 至成微, 朗力半导体, 速通智联 --out ./competitors/2026-05/
...
**Bridging back to portfolio-tracking**: cards produced here use the **same schema** as those generated inside `/portfolio-tracking`. To reuse them in a quarterly report, copy them into `./portfolio/<slug>/competitors/` with the `{NN}_{name-slug}.md` naming convention — the parent command's Step 6.2 will detect the cache and skip re-research for those competitors.
...
Output: annotated DD report at `./deals/techdd/han-wu-ji/`.
...
Output: structured industry report at `./research/<industry-slug>/<YYYYMMDD>_industry-report.md`.
...
- Re-install **does NOT touch your output or input files**. The per-domain report dirs (`./deals/`, `./portfolio/`, `./intel/`, `./research/`) and your `./workspace/inbox/` are yours.
...
- **Your output dirs** (`./deals/`, `./portfolio/`, `./intel/`, `./research/`) and **`./workspace/inbox/`** — reports, evidence, inboxes
...
# Remove your generated artifacts + inbox (DESTRUCTIVE — back up first!)
# rm -rf ./deals/ ./portfolio/ ./intel/ ./research/ ./workspace/
```
