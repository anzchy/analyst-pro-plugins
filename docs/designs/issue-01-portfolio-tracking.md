...
| P5 | 输入：合并报表（必备）+ 上期报告 / 董事会材料 / 访谈 / 新闻（可选）；输出 `./workspace/state/portfolio/{slug}/{YYYYQX}_post_investment_tracking.md` | ✅ confirmed |
...
2. **竞对名单跨季度沉淀**：✅ 持久化到 `./workspace/state/portfolio/{slug}/competitors.yml`。每季度从此读取作为默认值，AskUserQuestion 时允许增删；最新名单回写覆盖。
3. **章节一项目概况**：✅ 首次手工录入并存到 `./workspace/state/portfolio/{slug}/project_baseline.yml`，后续季度直接复用，不再询问。
