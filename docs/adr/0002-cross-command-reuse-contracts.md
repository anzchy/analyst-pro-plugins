...
## Amendment 2026-05-17 — 闸门2 user-D1 自述绕闸口收口（codex-toolkit 审计残留缺口）

**性质**：实现加固兼**文档化逃生口语义收紧**——`.fin-cache` 路径/命名/复用
契约**不变**；收紧的是 `financial-analyzer.md` 1.2.2 闸门 2 对「非季末缓存键」
的**唯一放行口**判定，使 fin-sidecar period-key 冻结契约真正不可被自述绕过。

**动因**：`/codex-toolkit:audit --full` 复核 `.fin-cache` / `./competitors/`
两修复时，在 period-key 硬闸门里查出一条残留缺口（审计 F1）：原闸门 2 只要
`period_source=user-D1` 即放行任意非季末 `corrected_period`，而 `period_source`
由上游 LLM 决策记录自填、无 PDF/用户二次背书——把文件名交付日改标
`user-D1` 即可让交付日伪装成报告期落进 `.fin-cache` 键，正是已修
period-key bug 的同类回流路径，只是换了入口。

**变更**：决策记录新增 auditable 字段 `user_confirmed_non_quarter`（仅当分析师
在 D1 中**显式确认**该期确为真·非季末特殊报表时为 `true`；LLM 不得自置）。
闸门 2 改为：非季末 `corrected_period` 仅在 `period_source=user-D1`
**且** `user_confirmed_non_quarter is True` **且** 仍为 8 位 `YYYYMMDD` 三条同时
成立才放行，否则一律 `GATE_FAIL`。8 场景 feedback-loop 验证：绕闸尝试被拦、
合法非季末逃生口仍 GATE_OK、原 period-key bug 仍被拦、正常季末/ pdf-verified
路径不回归。

**同步落点**：本 ADR + `commands/financial-analyzer.md`（1.2.1 决策记录 schema
+ 1.2.2 闸门 2 代码 + 逃生口 prose）。`docs/designs/fin-sidecar-contract.md`
**不动**（JSON shape 未变，仅写入键的准入判定更严）。审计同批发现的 F2/F3
（`portfolio-tracking.md` 旧嵌套枚举 / D3 目录覆盖两处 prose 仍写 glob 而代码
已 find）、F4（`competitor-enricher.md` 引言默认目录与 Step 0.2 不一致）属
spec 一致性修整，随本批一并落（无契约面）。仍须 source↔runtime cache 双端同步。

**未触动**：`.fin-cache` 扁平路径/命名/复用契约；`<YYYYMMDD>` token 派生其余
环节；JSON schema；竞对档案契约；ADR 已冻结条款。本条仅收紧闸门 2 的非季末
放行准入，未新增/移除闸门，亦未改变 GATE_OK→放行 / GATE_FAIL→hard-stop 语义。
