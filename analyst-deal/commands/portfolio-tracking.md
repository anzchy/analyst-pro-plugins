...
mkdir -p ./workspace/state/portfolio/{slug}/competitors/   # Step 6 单家竞对档案缓存
```
...
**Default scan dir:** `./workspace/state/portfolio/{slug}/` (where `{slug}` is the project slug computed in Step 0.2 — e.g., `矽昌通信` 或 `deepfusion-energy`).
...
```bash
SCAN_DIR="./workspace/state/portfolio/{slug}"
...
D3 — 本季度材料路径
Project/branch/task: 投后跟踪 — {公司名} {季度}
ELI10: 命令在 ./workspace/state/portfolio/{slug}/ 下扫描了一下，下面是找到的候选文件。如果对就 A，否则 B 手工指定。
...
A) 用上面自动扫描的结果（recommended）
  ✅ 零键入；命令直接进入抽取与调度阶段
  ✅ 保证所有路径都在 ./workspace/state/portfolio/{slug}/ 下，与 baseline + competitors.yml 同目录方便后续追溯
  ❌ 如果有材料放在其他目录，会被漏掉
B) 手工指定路径（覆盖默认）
...
如选 **A**：
- 若自动扫描中**未发现合并报表**（候选数 = 0），不接受 A — 强制提示用户："至少需要 1 个合并报表 PDF/xlsx，请把文件放到 `./workspace/state/portfolio/{slug}/` 后重跑，或选 B 手工指定路径。" 然后 abort 或回到 D3。
...
YAML 输出路径: ./workspace/state/portfolio/{slug}/current_quarter_financials.yml
```
...
把 Step 5 写出的 `current_quarter_financials.yml` 增量同步到 `./workspace/state/portfolio/{slug}/*历年财务报表*.xlsx`，保证投后报告引用的历年数据与本季度同步。

### 5.5.1 目标 xlsx 检测 + 锁文件保护

```bash
SLUG_DIR="./workspace/state/portfolio/{slug}"
...
- `{YAML_PATH}` ← `./workspace/state/portfolio/{slug}/current_quarter_financials.yml`
...
```
./workspace/state/portfolio/{slug}/competitors/{NN}_{name-slug}.md
```
...
如 `./workspace/state/portfolio/{slug}/competitors/{NN}_{name-slug}.md` 已存在且与 `competitors.yml` 当前条目匹配，主 Agent 在该批内**跳过**对该家的 Agent dispatch，直接 Read 已缓存的 card 走 6.1 (b)(c) 流程把它追加到主报告。仅当用户在 Step 2.1 选择 "C) 全部重写" 或显式手工删除缓存时才重新调研。
...
2. 单家竞对档案缓存目录：`./workspace/state/portfolio/{slug}/competitors/`（列出本次写入 / 复用的文件名）
