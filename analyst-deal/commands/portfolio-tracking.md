...
4. 行业发展（分批并发 dispatch N × `competitor-enricher` → 每家返回即写 `./competitors/{name}.md` 缓存 + Edit 追加主报告）
...
mkdir -p ./competitors/   # Step 6 单家竞对档案缓存（ADR 0002 2026-05-17：浅层规范位置）
```
...
**同时读取顶层有序键 `档案搜索路径:`**（竞对档案块复用的全局有序回退链，Step 6 按此顺序逐路径查找）。若文件中**缺该键**（旧版 competitors.yml），**静默**采用默认 `[./competitors/, ./, ./portfolio/{slug}/competitors/]`，不报错、不弹问题（Step 9 会回写补全）。`./competitors/` 居首是 ADR 0002 2026-05-17 修订后 standalone `/competitor-enricher` 的默认产出位；后两条为向后兼容旧产物散落位。
...
档案搜索路径:            # 全局有序回退链；Step 6 对每家竞对按此顺序逐路径查档案块，全链未命中才走 jina
  - ./competitors/        # standalone /competitor-enricher 默认产出位（ADR 0002 2026-05-17）
  - ./                    # 向后兼容：旧 standalone 默认 CWD 根
  - ./portfolio/{slug}/competitors/   # 向后兼容：旧主命令缓存位
竞对列表:
  - 名称: <str>
    备注: <str>          # 可选
  - ...
...
Run via Bash to discover candidate inputs by filename pattern; collect
non-empty results into in-memory candidate lists per material type.

> **必须用 `find`，不能用 `ls "$DIR"/*pat`。** 本命令的 Bash 在 zsh 下执行：
> 多 pattern 的 `ls a* b* c*` 只要**任一** glob 未匹配，zsh 会在 `ls` 运行
> 前整体报 `no matches found` 并**丢弃已匹配的兄弟 pattern 结果**（`2>/dev/null`
> 抑制不了 shell 的 glob 展开报错）。即"有合并报表 PDF 但无 xlsx 兄弟文件"
> 这一常见情形下整块扫描返回空 → 全流程误判无财务输入。`find … -name`
> 自己处理通配、目录为空/不存在均 rc 安全。详见 ADR 0002 同款修复与
> 记忆 `skill-bash-runs-under-zsh-nomatch`。
...
find "$SCAN_DIR" -maxdepth 1 -type f \( \
       -name '*合并报表*.pdf' -o -name '*合并报表*.xlsx' \
    -o -name '*财务报表*.pdf' -o -name '*财务报表*.xlsx' \
    -o -name '[0-9][0-9][0-9][0-9][0-1][0-9][0-3][0-9]*报表*.pdf' \) 2>/dev/null

# 上期投后报告（生成产物 + 手工归档版本）
find "$SCAN_DIR" -maxdepth 1 -type f \( \
       -name '*投后跟进报告*.md' -o -name '*投后跟进报告*.pdf' -o -name '*投后跟进报告*.docx' \
    -o -name '*投后跟踪*.md'     -o -name '*post_investment_tracking*.md' \) 2>/dev/null

# 董事会材料（直属文件 + board-materials* 子目录内文件）
find "$SCAN_DIR" -maxdepth 2 -type f \( \
       -name '*董事会*.md' -o -name '*董事会*.pdf' -o -name '*board*.md' \
    -o -path '*/board-materials*/*' \) 2>/dev/null

# 访谈纪要
find "$SCAN_DIR" -maxdepth 1 -type f \( \
       -name '*访谈*.md' -o -name '*交流*.md' -o -name '*interview*.md' \) 2>/dev/null

# 新闻 / 舆情（直属文件 + news-clippings* 子目录内文件）
find "$SCAN_DIR" -maxdepth 2 -type f \( \
       -name '*新闻*.md' -o -name '*舆情*.md' -o -name '*news*.md' \
    -o -path '*/news-clippings*/*' \) 2>/dev/null
```
...
# find（非 glob）：$SLUG_DIR 常无历年 xlsx（prose 显式按非错误处理），
# zsh 下 `ls "$SLUG_DIR"/*…*.xlsx` 未匹配会向日志喷 `no matches found`
# （2>/dev/null 抑制不了 shell 的 glob 展开报错）。grep -v '/~\$' 排除
# basename 以 ~$ 开头的 Excel 锁文件（旧 '^~\$' 锚行首，对全路径输出从不命中）。
XLSX=$(find "$SLUG_DIR" -maxdepth 1 -type f -name '*历年财务报表*.xlsx' 2>/dev/null | grep -v '/~\$' | head -1)
if [ -z "$XLSX" ]; then
...
分析师常先用 standalone `/analyst-deal:competitor-enricher` 生成竞对档案块，再跑本
投后命令。ADR 0002 2026-05-17 修订后 standalone 默认产出 `./competitors/<name-slug>.md`
（去 `{NN}_` 前缀）；历史产物可能仍散落 CWD 根或 `./portfolio/{slug}/competitors/`。
竞对档案块（内容符合 `competitor_card_schema.md`）是**冻结的跨命令契约**
...
`档案搜索路径`（默认 `[./competitors/, ./, ./portfolio/{slug}/competitors/]`）**逐路径**查找：

1. `name-slug` = 竞对名按 Step 0.2 同规则做 slug（保留中文，去空格 + 特殊字符）。
2. 在当前路径 P（**非递归**，仅该目录直属 `*.md`）按下面规则匹配：
   - **主规则**：basename **小写后包含小写 name-slug** 即命中。新规范文件名
     无前缀（`英伟达.md`）；该规则同时向后兼容旧 `NN_` 数字前缀（`01_英伟达.md`
     也命中 `英伟达`）——故去前缀不破坏对旧产物的复用。
...
# 用 find（非 glob）枚举：$P 可能为空目录或不存在，zsh 下未匹配 glob
# （ls "$P"/*.md）会在 ls 运行前就 `no matches found` 报错——find 不会。
find "$P" -maxdepth 1 -type f -name '*.md' 2>/dev/null | while read -r f; do
  b="$(basename "$f")"
...
```
./competitors/{name-slug}.md
```

- ADR 0002 2026-05-17 修订：单一规范位置 `./competitors/`，**去 `{NN}_` 前缀**，
  与 standalone `/competitor-enricher` 写入位完全一致（写=读同一路径，不再分散
  到 `./portfolio/{slug}/competitors/`）。报告章节四顺序由 `competitors.yml`
  迭代序决定、与文件名无关，去前缀不影响排序。
- `name-slug`：竞对名按 0.2 同样规则做 slug（保留中文，去空格 + 特殊字符）
- 内容**原样**写入 enricher 返回的 markdown，不加额外 frontmatter / 包裹
- 目标文件已存在 → 覆盖（本季度最新调研即权威；旧档案已在 6.0 复用判定阶段处理）
...
直接 Read 其解析到的档案块文件（在 6.0 记录的「来源路径 P」下，规范位 `./competitors/`，
向后兼容 CWD 根 / `./portfolio/{slug}/competitors/`、或用户经 D5-B 指定的路径），走 6.1 (b)(c)
流程把它追加到主报告，并记下该家的**来源路径**供 Step 8 / Output 逐家标注。

中断恢复是本机制的特例：上次跑出的 `./competitors/{name-slug}.md`
属默认 `档案搜索路径` 首条，自然被 6.0 有序回退链命中复用，无需单独逻辑。
...
- D5 选 A / C，或 Step 2.1 读出时该键缺失（旧版 yml）→ 回写当前生效列表
  （即默认 `[./competitors/, ./, ./portfolio/{slug}/competitors/]` 或文件中既有值），**补全**该键。
...
2. **竞对档案逐家来源表**（每家一行，来自 Step 8 检查 5）：
   ...
   单家档案默认缓存目录为 `./competitors/`（ADR 0002 2026-05-17；写=读同一路径）。
