...
for r in RECORDS:
    ...
    # 闸门 2：缓存键（corrected_period）非季末日且非 user-D1 → 拦截
    if src != "user-D1":
        if len(corr) != 8 or corr[4:8] not in QE:
...
- 真·非季末特殊报表（文档自述就是月中等非季末日，罕见）：仅当 `period_source =
  pdf-verified` 时才允许，且必须走 user-D1 显式确认该期为 key（闸门 2 放行口）。
  绝不允许由未经 PDF 核验的文件名 token 落到这一支。
