...
- All user-facing artifacts (reports, evidence) write to a shallow per-domain dir under the user's current working directory (`./research/`); the command creates it with `mkdir -p`. User-supplied inputs stay in `./workspace/inbox/`.
